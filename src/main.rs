...
enum Commands {
    ...
    /// Configure personality, behavior and autonomy settings
    Personality {
        #[command(subcommand)]
        personality_command: PersonalityCommands,
    },
...
enum PersonalityCommands {
    /// Run the interactive personality wizard
    Wizard,
    /// Show current personality configuration
    Show,
    /// Set personality profile (balanced, ops, mentor, forensic, creative)
    Profile {
        /// Profile name
        name: String,
    },
    /// Set a specific trait (precision, warmth, boldness, brevity, curiosity, skepticism)
    Trait {
        /// Trait name
        name: String,
        /// Value 0-100
...
async fn main() -> Result<()> {
    ...
    // Filter out verbose skill warnings by default
    let default_filter = if std::env::var("RUST_LOG").is_ok() {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info"))
    } else {
        EnvFilter::new("info,zeroclaw::skills=warn")
    };
    ...
    // Personality configuration (wizard, show, or set traits)
    if let Commands::Personality {
        personality_command,
    } = &cli.command
    {
        match personality_command {
            PersonalityCommands::Wizard => {
                let config = tokio::task::spawn_blocking(onboard::run_personality_wizard)
                    .await
                    .context("Failed to run personality wizard")?
                    .context("Wizard failed")?;

                println!("Personality wizard completed: profile={}", config.profile);
            }
            PersonalityCommands::Show => {
                println!("Personality configuration:");
                println!("  Use: zeroclaw personality wizard");
            }
            PersonalityCommands::Profile { name } => {
                println!("Setting profile to: {}", name);
            }
            PersonalityCommands::Trait { name, value } => {
                println!("Setting trait {} = {}", name, value);
            }
        }
        return Ok(());
    }
    ...
    match cli.command {
        Commands::Onboard { .. } | Commands::Completions { .. } | Commands::Personality { .. } => {
            unreachable!()
        }
